...
import com.cjnoyessw.twitter.hadoop.mapper.ScreennameFrequencyMapper;


public class ScreennameFrequencyDriver extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		conf.setJobName("ScreennameFreqency");
		FileInputFormat.addInputPath(conf, new Path(args[0]));
		FileOutputFormat.setOutputPath(conf, new Path(args[1]));
		conf.set("analyze_col", args[2]);
		conf.setOutputKeyClass(Text.class);
		conf.setOutputValueClass(IntWritable.class);
		conf.setMapperClass(ScreennameFrequencyMapper.class);
	...
	public static void main(String [] args) throws Exception {
		int exitCode = ToolRunner.run(new ScreennameFrequencyDriver(), args);
